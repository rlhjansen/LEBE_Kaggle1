...
def store_time(_float, path):
    with open(path, "w") as time_file:
        time_file.write(str(_float))
...
def test_regressor(regr, data, labels):
    ...
    return np.sum(np.power(np.log(pred + 1) - np.log(labels + 1), 2)) / len(
        labels)
...
#        for batch in range(num_batches):
        for batch in range(num_batches):
            ...
            print("Loading data...")
            try:
                train_data, train_labels = load_train(PATH_INPUT_TRAIN,
                                                  batch * BATCH_SIZE,
                                                  BATCH_SIZE)
    ...
    for batch in range(num_test_batches):
        ...
        # print(prediction)
        # print(IDs)
        store_submission(tru_prediction, tru_IDs, PATH_TEST_OUT + str(len(layers)) + POST_PATH_CSV)
    store_time(time() - sub_start, PATH_TIME_SAVE_LAYERS + str(len(layers)) + POST_PATH_TIME)

    #print("Done!\nThe regression has an average error of:", np.sqrt(new_err))

main_pre_processing()
MAX_TRIES = 10
TRAIN_SIZE = sum(1 for _ in open(PATH_INPUT_TRAIN))
layerlist = [[100,100,100,100,100],[100,100,100,100,100,100,100,100,100,100]]

"""
for i in layerlist:
    with open(PATH_TEST_OUT +str(len(i))+POST_PATH_CSV, "a") as out_file:
